...
def plot_dispersion_curve(dispersion_curve):
    ...
    fig, ax = plt.subplots()
    # Plot the dispersion curve
    ax.plot(dispersion_curve[:, 0], dispersion_curve[:, 1])
    # Add labels
...
def pick_dispersion_curves(dispersion_curve, eps=0.5, min_samples=5):
    """
...
def tikhonov_inversion(dispersion_curve, alpha=0.1):
    """
...
def evaluate_velocity_model(velocity_model, dispersion_curve):
    """
    Evaluate the accuracy of the estimated velocity model by calculating the RMSE between the predicted and actual dispersion curves.
    ...
    Returns:
    rmse (float): The RMSE between the predicted and actual dispersion curves.
    """
    ...
    predicted_dispersion_curve = forward_model(velocity_model)

    # Calculate the RMSE between the predicted and actual dispersion curves
    rmse = np.sqrt(mean_squared_error(dispersion_curve[:, 1], predicted_dispersion_curve))
...
def plot_velocity_model(velocity_model, dispersion_curve):
    """
    Plot the estimated velocity model.
    ...
    ax.plot(dispersion_curve[:, 0], velocity_model, label='Estimated velocity model')
